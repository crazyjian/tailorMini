...
Page({
  ...
  generate:function() {
    ...
    if(flag) {
      ...
      wx.request({
        url: app.globalData.backUrl + '/erp/minigenerateothertailordatacombine',
        data: {
        ...
        success: function (res) {
          ...
          if (res.statusCode == 200 && res.data.tailorList) {
            wx.navigateTo({
              url: "../otherFixedSave/otherFixedSave?tailorList=" + encodeURIComponent(JSON.stringify(res.data.tailorList))
            })
          } else {
            wx.showToast({
              title: "服务器发生异常",
